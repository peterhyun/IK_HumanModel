...
public:
    ...
    bool solveIKPelvis(const glm::vec3& target, const glm::quat& ori_d);
...
private:
    ...
    glm::quat oldPelvisOri_d = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
    glm::vec3 oldtarget = glm::vec3(1.0f);
    glm::vec3 oldPelvisTarget = glm::vec3(0.0f);
...
//Gets pelvis loc & pelvis ori. However, the true target is actually the feet. It's actually solving IK for the feet
bool BoneRig::solveIKPelvis(const glm::vec3& pelvisPos, const glm::quat& pelvisOri_d){
    if(oldPelvisOri_d == pelvisOri_d && oldPelvisTarget == pelvisPos)
        return true;
    //std::cout << "Different target for solveIKPelvis()!" << std::endl;
    oldPelvisOri_d = pelvisOri_d;
    oldPelvisTarget = pelvisPos;
    
    //Let's first solve the IK for the left foot. We get the current left foot position&ori.
    //This will be our TARGET!!!
    glm::vec3 leftFootPos = getEndEffectorLeftFootP();
    glm::quat leftFootOri_d = getEndEffectorLeftFootQ();
    glm::vec3 rightFootPos = getEndEffectorRightFootP();
    glm::quat rightFootOri_d = getEndEffectorRightFootQ();
    
    //Our pelvis will now move. This will be fixed from now on.
    glm::mat4 Translate = glm::mat4(1.0f);
    Translate = glm::translate(Translate, pelvisPos);
    Joints[0].setT(Translate);
    Joints[0].setR_quat(pelvisOri_d);
    
    float error = 0.0f;
    Eigen::VectorXf deltaQ(4);
    Eigen::VectorXf Q_desired(4);
    Q_desired << 0,0,0,0;
    glm::vec3 endEffectorP;
    glm::quat endEffectorOri;
    glm::vec3 errorP;
    ...
    float deltaTime = 0.01;
    Eigen::MatrixXf J_plus(4,6);
    
    Eigen::VectorXf oldQ_desired(4);
    float old_error;
    ...
    while(true){
        endEffectorP = getEndEffectorLeftFootP();
        endEffectorOri = getEndEffectorLeftFootQ();
        
        errorP = leftFootPos - endEffectorP;
        
        Eigen::VectorXf x_dot(6);
        x_dot = getx_dot(deltaTime, errorP, endEffectorOri, leftFootOri_d);
        J = getJacobianLeftLeg(endEffectorP);
        J_plus = ((J.transpose() * J + 0.1*0.1*Eigen::Matrix<float,4,4>::Identity()).inverse()) * J.transpose();
        deltaQ = J_plus * x_dot;
        
        oldQ_desired = Q_desired;
        Q_desired += deltaQ;
        
        setOrientationLeftLeg(Q_desired);
        
        glm::quat qd= glm::inverse(leftFootOri_d) * endEffectorOri;
    ...
    iter = 0;
    while(true){
        endEffectorP = getEndEffectorRightFootP();
        endEffectorOri = getEndEffectorRightFootQ();
        
        errorP = rightFootPos - endEffectorP;
        
        Eigen::VectorXf x_dot(6);
        x_dot = getx_dot(deltaTime, errorP, endEffectorOri, rightFootOri_d);
        J = getJacobianRightLeg(endEffectorP);
        J_plus = ((J.transpose() * J + 0.1*0.1*Eigen::Matrix<float,4,4>::Identity()).inverse()) * J.transpose();
        deltaQ = J_plus * x_dot;
        ...
        setOrientationRightLeg(Q_desired);
        
        glm::quat qd= glm::inverse(rightFootOri_d) * endEffectorOri;
